...
export function createPostPreview(post) {
  ...
  let isBlacklisted =
    post.flags?.deleted || post.tags?.general?.includes("young");

  if (!imageUrl && post.file?.md5) {
    imageUrl = `https://static1.e621.net/data/sample/${post.file.md5.substring(
      0,
      2
    )}/${post.file.md5.substring(2, 4)}/${post.file.md5}.jpg`;
  }
