...
export class BaseTool {
  ...
  getElements() {
    ...
    this.advancedOptionsDetails = document.getElementById(
      "advanced-options-details"
    );
    this.favoriteModeSelect = null;
  }

  renderAdvancedOptions() {
    if (!this.advancedOptionsDetails) return;

    this.advancedOptionsDetails.innerHTML = `
    <label class="inline-flex items-center mb-2">
      <input type="checkbox" id="useDefaultBlacklist" class="mr-2" ${
        this.useDefaultBlacklistCheckbox?.checked ? "checked" : ""
      }>
      Use Default Blacklist
    </label>
    <div class="flex mb-2">
      <input type="text" id="blacklist-input" class="editor-input flex-1 mr-2" placeholder="Add blacklist tag">
      <button type="button" id="add-blacklist-btn" class="btn">Add</button>
    </div>
    <div id="blacklist-tags-container" class="flex flex-wrap gap-1"></div>
  `;

    // Re-fetch elements inside advanced options after rendering
    this.useDefaultBlacklistCheckbox = document.getElementById(
      "useDefaultBlacklist"
    );
    this.blacklistInput = document.getElementById("blacklist-input");
  ...
  init() {
    this.loadBlacklist();
    this.insertFavoriteModeSelector();
    this.initializeLazyLoader();
    this.attachEventListeners();
  }

  insertFavoriteModeSelector() {
    // Only show for Mass Favorite tool
    if (this.config?.title !== "Mass Favorite Tool") return;
    ...
    wrapper.innerHTML = `
    <label class="editor-label">Mode</label>
    <select id="favorite-mode" class="editor-input mb-3">
      <option value="favorite">Mass Favorite (filtered)</option>
      <option value="remove">Remove Blacklisted Favorites</option>
    </select>
  `;

    // Insert ABOVE the start action button
    this.actionButtonContainer.prepend(wrapper);

    this.favoriteModeSelect = wrapper.querySelector("#favorite-mode");
  }
  ...
  async fetchPosts(isFetchingMore = false) {
    ...
    const data = await apiRequest(
      `posts.json?tags=${encodeURIComponent(
        currentTags
      )}&limit=${postLimit}&page=${this.currentPage}`,
      credentials
    );
