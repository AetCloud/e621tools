...
export async function apiRequest(endpoint, credentials, options = {}) {
  ...
  try {
    ...
    if (response.status === 201 || response.status === 204) {
      ...
      // ### FIX: Parse the JSON body on 201 (Create) responses ###
      if (response.status === 201) {
        try {
          const data = await response.json();
          logger.log(`[API Success] Parsed 201 response.`, data);
          return data;
        } catch (e) {
          logger.warn("[API Warn] 201 response had no JSON body.");
          return { success: true };
        }
      }
      return { success: true };
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        case 401:
          errorMessage =
            "Authentication failed. Please check your username and API key in the settings.";
          break;
        case 403:
          errorMessage =
            "You do not have permission to access this resource. Your account may be blocked or you may be trying to access a resource that you are not allowed to.";
          break;
        case 404:
        ...
        case 412:
          ...
          errorMessage = `Precondition Failed: ${
            errorData.message ||
            "The server rejected the request. This could be a duplicate image."
          }`;
          break;
        case 422:
        ...
        case 429:
          errorMessage =
            "You are being rate-limited. Please wait a few moments before trying again.";
          break;
        case 500:
          errorMessage =
            "The e621.net server encountered an internal error. Please try again later.";
          break;
        case 503:
          errorMessage =
            "The e621.net server is currently unavailable. Please try again later.";
          break;
      }
...
export async function apiUploadRequest(endpoint, credentials, formData) {
  ...
  if (!username || !apiKey) {
    return {
      success: false,
      error: "Username and API key are required for uploads.",
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      const errorReason =
        ...
        (responseData.errors
          ...
          : "Unknown error");
      logger.error(
        `[API Upload Error] Status: ${response.status}. Reason: ${errorReason}`
      );
    ...
    logger.error(
      `[API Upload Fetch Error] A critical error occurred during the upload to ${url}:`,
      error
    );
    return { success: false, error: error.message };
  }
